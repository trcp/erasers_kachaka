...
{
public:
    EmcJoyNode() : Node("emc_joy"), button_pressed_(false)  // 初期状態はfalse
    {
        // サービスクライアントを作成
    ...
    void joy_callback(const sensor_msgs::msg::Joy::SharedPtr msg)
    {
        ...
        if (msg->buttons.size() > 0)
        {
            if (msg->buttons[0] == 0 && !button_pressed_)
            {
                ...
                {
                    RCLCPP_WARN(this->get_logger(), "Emergency service not available");
                    return;
                }
                ...
                RCLCPP_INFO(this->get_logger(), "Emergency service called");
            ...
            else if (msg->buttons[0] == 1 && button_pressed_)
            {
                ...
                RCLCPP_INFO(this->get_logger(), "Button released, Prease push Robot's power button.");
            }
        }
    }
